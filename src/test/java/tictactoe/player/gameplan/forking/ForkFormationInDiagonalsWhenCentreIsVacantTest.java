...
public class ForkFormationInDiagonalsWhenCentreIsVacantTest {
    ...
    public ForkFormationInDiagonalsWhenCentreIsVacantTest(Row top, Row middle, Row bottom, int index) {
        this.topRow = top;
    ...
    public static Collection dataSetup() {
        return Arrays.asList(new Object[][]{
                {
                        aRowBuilder().withHorizontalRow(X, VACANT, O, 0).build(),
                ...
                {
                        aRowBuilder().withHorizontalRow(O, VACANT, X, 0).build(),
                        aRowBuilder().withHorizontalRow(VACANT, VACANT, O, NUMBER_OF_CELLS_IN_ROW).build(),
